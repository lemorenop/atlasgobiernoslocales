...
export default function SearchComparative({
    lang,
    label = "",
    title = "",
    subtitle = "",
    ...
    useEffect(() => {
      const fetchResults = async () => {
        if (query.length > 1) {
          setIsLoading(true);
          try {
            const response = await fetch(`/api/search?query=${encodeURIComponent(query)}&lang=${encodeURIComponent(lang)}${countryCode ? `&countryCode=${encodeURIComponent(countryCode)}` : ''}${nivel ? `&nivel=${encodeURIComponent(nivel)}` : ''}`);
            if (!response.ok) {
