...
export default function Select({ options, onChange, lang, selected ,id,defaultAllLabel,label}) {
  return (
    <div className="w-full">
      <Listbox value={selected} onChange={onChange}>
        <ListboxButton
          className={`${label? "font-bold text-blue-CAF":"text-placeholder"} w-full md:w-80 inline-flex items-center gap-2  bg-white border-1 border-black px-3  shadow-inner shadow-white/10 focus:outline-none  data-[focus]:outline-1 data-[focus]:outline-white cursor-pointer  justify-between data-[open]:rotate-0 py-s`}
        >
          {label?label:defaultAllLabel && selected[id]==='all' ? defaultAllLabel : selected[`name_${lang}`]}
